...
(async () => {
  ...
  for (b of browsers) {
    ...
    // Tambien guardar la ultima ejecucion par aparte para poder levantar un webserver
    const dirToServe = "./results/last";
    deleteFolderRecursive(dirToServe);
    try {
      fse.copySync(basePath, dirToServe, { overwrite: true });
      console.log("success!");
    } catch (err) {
      console.error(err);
    }
...
function createTree() {
  ...
  fs.writeFileSync(graphFilenameRoot + ".json", json, function (err) {
    if (err) throw err;
...
function createErrorGraph() {
  ...
  fs.writeFileSync(graphFilenameRoot + "2.json", json, function (err) {
    if (err) throw err;
  });
  ...
  fs.writeFileSync(graphFilenameRoot + "3.json", newJson, function (err) {
    if (err) throw err;
  });
}
